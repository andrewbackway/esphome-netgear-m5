...
namespace esphome
{
    namespace netgear_m5
    {
        ...
        void NetgearM5Component::publish_pending_()
        {
            ...
            // Use JsonDocument with DynamicAllocator for ArduinoJson 7
            ArduinoJson::JsonDocument doc(ArduinoJson::DynamicAllocator(16384)); // 16KB buffer
            ...
            for (auto &b : this->num_bindings_)
            {
                if (!b.sensor)
                    continue;
                std::string v = dotted_lookup_(b.path, root); // Fixed syntax: removed 'tutela'
                if (!v.empty())
